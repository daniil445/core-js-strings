...
function removeFirstOccurrences(str, value) {
  const num = value.substring(value.indexOf(value), value.length);
...
function removeLastOccurrences(str, value) {
  ...
  return num >= 0
    ? str.slice(0, num).concat(str.slice(num + value.length))
    : str;
}
...
function countVowels(str) {
  ...
  let sum = 0;
  while (i < str.length) {
    sum += ['a', 'e', 'i', 'o', 'u', 'y'].includes(str.toLowerCase().charAt(i))
      ? 1
      : 0;
    i += 1;
  }
...
function isPalindrome(str) {
  const mir = str
    ...
    .reverse()
    .join('');
  return mir === str.toLowerCase().replace(/[^a-zA-Z0-9]+/g, '');
}
...
function reverseWords(str) {
  ...
  str.split(' ').forEach((word) => {
    text += word.split('').reverse().join('').concat(' ');
  });
...
function invertCase(str) {
  let text = '';
  str.split('').forEach((charCheck) => {
    if (/[A-Z]/.test(charCheck)) {
      text += charCheck.toLowerCase();
    } else if (/[a-z]/.test(charCheck)) {
      text += charCheck.toUpperCase();
    } else {
      text += charCheck;
    }
  });
  ...
  str.split('').forEach((charCheck) => {
    if (/[a-z]/.test(charCheck)) {
      text += String.fromCharCode(
        charCheck.charCodeAt() + 13 > 122
          ? charCheck.charCodeAt() + 13 - 26
          : charCheck.charCodeAt() + 13
      );
    } else if (/[A-Z]/.test(charCheck)) {
      text += String.fromCharCode(
        charCheck.charCodeAt() + 13 > 90
          ? charCheck.charCodeAt() + 13 - 26
          : charCheck.charCodeAt() + 13
      );
    } else {
...
function getCardId(value) {
  ...
  if (/[2-9]/.test(value)) {
    ID += Number.parseInt(value.charAt(0), 10) - 1;
